...
public class ReceitaDAO {
	...
	public boolean existeRegistroPorIdReceita(int id) {
		...
		try {
			...
			System.out.println("Erro ao executar a verificaçãoo da existencia de receita por ID");
			System.out.println("Erro: "+ e.getMessage());			
		} finally {
			Banco.closeResultSet(resultado);
			Banco.closeStatement(stmt);
	...
	public int atualizarReceitaDAO(ReceitaVO receitaVO) {
		...
		try {
			...
			System.out.println("Erro ao executar a query de atualizaçãoo de receita");
			System.out.println("Erro: "+e.getMessage());;
		} finally {
			Banco.closeStatement(stmt);
			Banco.closeConnection(conn);
